  ...
  const sendMessage = useCallback(
    async (text: string, model: string, provider: string, images?: ImageAttachment[]) => {
      ...
      const markError = (raw: string): void => {
        ...
        // Only remove the assistant placeholder if it's still empty — never remove
        // a message that already has content (protects against timing issues).
        const conv = useConversationsStore.getState().conversations.find((c) => c.id === convId)
        const placeholder = conv?.messages.find((m) => m.id === assistantMsg.id)
        ...
        // Persistence is handled in the finally block to avoid races with addMessage persists
      }
      ...
      try {
        ...
        // Single persist point for both success and error — avoids races with
        // the addMessage persist that fires when the user message is first added.
